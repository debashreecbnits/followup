...
class TopConnection extends React.Component {
  constructor(props) {
    super(props);
    this.state = {
      favouriteList: [],
  ...
  getpotentialList() {
    ...
    Api.getApi('follow/'+this.props.userId, this.props.data.token).then(potentialListRes => {
      if (potentialListRes.ack == true) {
        this.setState({ showLoading: false, favouriteList: potentialListRes.FavouriteList })
      } else {
...
export default connect(mapStateToProps, null)(TopConnection);