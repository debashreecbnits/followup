  ...
  getpotentialList() {
    ...
    Api.getApi('follow/'+this.props.userId, this.props.data.token).then(potentialListRes => {
      if (potentialListRes.ack == true) {
        this.setState({ showLoading: false, favouriteList: potentialListRes.Following })
      } else {
    ...
    return (
      <React.Fragment>
        ...
        <TouchableOpacity onPress={()=>this.props.navigation.goBack()}>
          ...
          <View style={{ width:this.props.orientation == 'landscape'? '88%':'80%', alignItems: 'center' }}>
            <Text style={{
              color: "#74B9FF",
              fontSize: 20,
              fontWeight: "800",
            }}>Potential Connection List
            </Text>
          </View>
        </View>
...
export default connect(mapStateToProps, null)(PotentialConnection);